...
class CarouselWidget extends Widget
{
    ...
    public function init()
    {
        ...
        $this->activeImage = MarketingImage::find('marketingImagePath')
            ->where(['marketingImageIsActive' => 1])
            ->andWhere(['marketingImageIsFeatured' => 1])
            ->andWhere(['status_id' => 1])
            ->one();

        $this->images = MarketingImage::find()
            ->where(['marketingImageIsActive' => 0])
            ->andWhere(['marketingImageIsFeatured' => 1])
            ->andWhere(['status_id' => 1])
            ->orderBy('marketingImageWeight')
            ->all();
